...
const defRanges = [300, 400, 500, 600, 700, 800, 900, 1000, 2000, 3000]
...
function worldSalary (salary, { width = 960, height = 500, colors = defColors, colorRanges = defRanges, projectionKey = 'Mercator', styles = '' }) {
    ...
    salary.forEach((d) => { salaryHash[d.id] = +d.salary })
    topoData.features.forEach((d) => { d.salary = salaryHash[d.id] })
    ...
    svg.append('g')
        ...
        .attr('d', path)
        .attr('data-country', (d) => {
            return `${d.properties.name},${salaryHash[d.id]}`
        })
        .style('fill', function (d) { return color(salaryHash[d.id]) })
        .style('stroke', 'white')
